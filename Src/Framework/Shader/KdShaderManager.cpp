...
{
	cbLight& light = m_cb9_Light.Work();
	light.ConeLight.Enable = _enable;

	m_cb9_Light.Write();
}

void KdShaderManager::WriteCBConeLight(const Math::Vector3& _pos, const Math::Vector3& _dir, const float _range, const float _angle, const Math::Vector3& _color)
{
	cbLight& light = m_cb9_Light.Work();
...
void KdShaderManager::SetConeLight(const Math::Vector3& _pos, const Math::Vector3& _dir, const float _range, const float _angle, const Math::Vector3& _color)
{
	ConeLight& coneLight = m_cb9_Light.Work().ConeLight;

	coneLight.Pos = _pos;
	coneLight.Dir = _dir;
	coneLight.range = _range;
	coneLight.angle = _angle;
	coneLight.Color = _color;
