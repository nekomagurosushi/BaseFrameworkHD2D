...
void KdDirect3D::ConvertWorldToScreenDetail(const KdCamera& camera, const Math::Vector3& pos, Math::Vector3& result)
{
	// ビューポートを取得
	Math::Viewport vp;
	CopyViewportInfo(vp);
	...
	Math::Matrix wvp = world * camera.GetCameraMatrix().Invert() *
	...
	// ここで幅や高さ考慮をして計算する(これで正確なスクリーン座標になる)
